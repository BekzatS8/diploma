...
func paths() map[string]any {
	return map[string]any{
		...
		"/api/v1/client/orders/{id}/review": pathItem(
			getOp("Reviews", "Get client-to-executor order review", "Client/admin reads the client review for an owned completed order.", true, []any{path("id", "string", "Order UUID.")}, nil, ok("Review")),
			postOp("Reviews", "Create client-to-executor review", "Client creates a review for the selected executor after a completed order.", true, []any{path("id", "string", "Order UUID.")}, body("CreateReviewRequest"), created("Review")),
		),
		"/api/v1/orders/{id}/review": post("Reviews", "Create counterparty order review", "Client reviews executor or executor reviews client for a completed order, depending on the authenticated role.", true, []any{path("id", "string", "Order UUID.")}, body("CreateReviewRequest"), created("Review")),
		...
		"/api/v1/my/reviews": get("Reviews", "My authored reviews", "Lists order and entity/course reviews created by the current user.", true, []any{pageParam(), pageSizeParam()}, nil, listOK("MyReviewsListResponse")),
		"/api/v1/my/reviews/{reviewId}": pathItem(
			getOp("Reviews", "Get my review", "Reads one authored review by id.", true, []any{path("reviewId", "string", "Review UUID.")}, nil, ok("MyReview")),
			patchOp("Reviews", "Patch my review", "Updates rating/comment for one authored order or entity review and recalculates ratings.", true, []any{path("reviewId", "string", "Review UUID.")}, body("CreateReviewRequest"), ok("MyReview")),
			deleteOp("Reviews", "Delete my review", "Soft-deletes one authored order or entity review and recalculates ratings.", true, []any{path("reviewId", "string", "Review UUID.")}, nil, ok("MyReview")),
		),
		...
		"/api/v1/users/{userId}/reviews":         get("Reviews", "User reviews", "Public paginated reviews received by any platform user.", false, []any{path("userId", "string", "User UUID."), pageParam(), pageSizeParam()}, nil, listOK("ReviewsListResponse")),
		...
		"/api/v1/courses/{id}/enroll": post("Courses", "Enroll in course", "Executor self-enrolls into a published course and appears in my courses.", true, []any{path("id", "string", "Course UUID.")}, nil, created("CourseAssignment")),
		...
		"/api/v1/my/course-assignments/enroll":                               post("Courses", "Enroll my course assignment", "Backward-compatible endpoint for executor self-enrollment by course_id.", true, nil, body("EnrollCourseRequest"), created("CourseAssignment")),
		...
		"/api/v1/my/courses":                                                 get("Courses", "My enrolled courses", "Executor list of enrolled courses with optional status filter.", true, []any{query("status", "string", false, "Assignment status."), pageParam(), pageSizeParam()}, nil, listOK("CourseAssignmentsListResponse")),
		"/api/v1/my/courses/completed":                                       get("Courses", "Completed course history", "Executor history of completed courses.", true, []any{pageParam(), pageSizeParam()}, nil, listOK("CourseAssignmentsListResponse")),
		"/api/v1/my/courses/{id}":                                            get("Courses", "My course learning view", "Executor reads one enrolled course with materials and progress. The id may be either course UUID or assignment UUID.", true, []any{path("id", "string", "Course or assignment UUID.")}, nil, ok("CourseLearningResponse")),
		"/api/v1/my/courses/{id}/materials/{materialId}/complete":            post("Courses", "Complete course material", "Alias for completing a material inside an enrolled course. The id may be either course UUID or assignment UUID.", true, []any{path("id", "string", "Course or assignment UUID."), path("materialId", "string", "Material UUID.")}, nil, ok("CourseAssignment")),
		"/api/v1/my/courses/{id}/mark-completed":                             post("Courses", "Complete enrolled course", "Alias for completing all materials in an enrolled course. The id may be either course UUID or assignment UUID.", true, []any{path("id", "string", "Course or assignment UUID.")}, nil, ok("CourseAssignment")),
...
func schemas() map[string]any {
	return map[string]any{
		...
		"Review":                obj(req("id", uuidStr()), req("order_id", uuidStr()), req("client_id", uuidStr()), req("executor_id", uuidStr()), req("reviewer_id", uuidStr()), req("reviewee_id", uuidStr()), req("reviewee_role", enumStr("client", "executor")), req("direction", enumStr("client_to_executor", "executor_to_client")), req("rating", intProp("Rating from 1 to 5.")), prop("comment", str("Review comment.")), req("created_at", dateTime()), req("updated_at", dateTime())),
		"ReviewsListResponse":   listSchema("Review"),
		"MyReview":              obj(req("id", uuidStr()), req("source", enumStr("order", "entity")), prop("order_id", uuidStr()), prop("target_type", reviewTargetTypeSchema()), prop("target_id", uuidStr()), prop("reviewee_id", uuidStr()), prop("reviewee_role", enumStr("client", "executor")), prop("direction", enumStr("client_to_executor", "executor_to_client")), req("rating", intProp("Rating from 1 to 5.")), prop("comment", str("Review comment.")), prop("metadata", freeObj("Review metadata.")), req("created_at", dateTime()), req("updated_at", dateTime())),
		"MyReviewsListResponse": listSchema("MyReview"),
		...
		"CourseDetailResponse":          obj(req("course", ref("Course")), req("materials", arr(ref("CourseMaterial"))), req("lessons", arr(ref("CourseLesson")))),
		"CourseLearningResponse":        obj(req("assignment", ref("CourseAssignment")), req("materials", arr(ref("CourseMaterial"))), req("lessons", arr(ref("CourseLesson")))),
		"CourseLesson":                  obj(req("key", str("Stable lesson key from material metadata.")), req("title", str("Lesson title.")), req("position", intProp("Lesson sort order.")), req("materials", arr(ref("CourseMaterial")))),
		"CreateMaterialRequest":         obj(req("title", str("Material title.")), prop("description", str("Material description.")), req("type", enumStr("video", "pdf", "link", "text")), prop("upload_id", uuidStr()), prop("url", str("URL for video/pdf/link.")), prop("content", str("Text content for text material.")), prop("position", intProp("Sort order.")), prop("duration_seconds", intProp("Video/material duration.")), prop("is_preview", boolProp("Preview availability.")), prop("metadata", freeObj("Optional lesson/block metadata, for example lesson_key, lesson_title, block_type."))),
		"UpdateMaterialRequest":         obj(prop("title", str("Material title.")), prop("description", str("Material description.")), prop("type", enumStr("video", "pdf", "link", "text")), prop("upload_id", uuidStr()), prop("url", str("URL for video/pdf/link.")), prop("content", str("Text content for text material.")), prop("position", intProp("Sort order.")), prop("duration_seconds", intProp("Video/material duration.")), prop("is_preview", boolProp("Preview availability.")), prop("metadata", freeObj("Optional lesson/block metadata, for example lesson_key, lesson_title, block_type."))),
		"CourseMaterial":                obj(req("id", uuidStr()), req("course_id", uuidStr()), req("title", str("Material title.")), prop("description", str("Material description.")), req("type", enumStr("video", "pdf", "link", "text")), prop("upload_id", uuidStr()), prop("url", str("Material URL.")), prop("content", str("Material content.")), req("position", intProp("Sort order.")), req("duration_seconds", intProp("Video/material duration.")), req("is_preview", boolProp("Preview availability.")), prop("metadata", freeObj("Lesson/block metadata.")), req("created_at", dateTime()), req("updated_at", dateTime())),
		...
		"EnrollCourseRequest":           obj(req("course_id", uuidStr())),
		...
		"CourseAssignment":              obj(req("id", uuidStr()), req("course_id", uuidStr()), req("executor_id", uuidStr()), prop("sanction_id", uuidStr()), prop("assigned_by", uuidStr()), prop("reason", str("Reason.")), req("source", enumStr("manual_admin", "self_enrolled", "sanction_low_rating_first", "sanction_low_rating_repeat")), req("status", enumStr("assigned", "in_progress", "completed", "overdue", "cancelled")), req("assigned_at", dateTime()), prop("due_at", dateTime()), prop("completed_at", dateTime()), req("created_at", dateTime()), req("updated_at", dateTime()), prop("progress", ref("CourseProgress"))),
