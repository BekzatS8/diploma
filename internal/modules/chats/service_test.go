...
func (m *mockRepo) CreateMessage(ctx context.Context, chatID, senderUserID, text string) (CreateMessageResult, error) {
	if m.createErr != nil {
...
func TestSendMessageMy_RejectsAdminAndEmpty(t *testing.T) {
	...
	if _, err := svc.SendMessageMy(context.Background(), "chat", "u1", "admin", "hello"); err != ErrForbidden {
		t.Fatalf("expected ErrForbidden for admin, got %v", err)
	}
	if _, err := svc.SendMessageMy(context.Background(), "chat", "u1", "executor", "   "); err != ErrInvalidInput {
		t.Fatalf("expected ErrInvalidInput for empty message, got %v", err)
	}
}

func TestListMessagesMy_OnlyParticipant(t *testing.T) {
	svc := NewService(&mockRepo{participant: false}, nil)
